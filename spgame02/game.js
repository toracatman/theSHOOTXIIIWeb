...
var boss = {
	...
	r: 6,
	//敵移動パターン
	m: [ [70, 0, -2], [144, 0, 2], [74, 0, -2], [50, -6, 0], [75, 4, 0], 
		[60, 0, -2], [20, 0, 2], [50, -6, 0], [75, 4, 0], [50, 0, 2], 
		[20, 0, -2], [50, -6, 0], [75, 4, 0], [20, 0, 0], [10, 0, 2] ],
	//現在参照しているパターン
	cp: 0,
	rp: 0,
		...
		for (var y = 0; y < this.r; y++) {
			for (var x = 0; x < this.c; x++) {
				var t = {
					...
					bounds_w: 32,
					bounds_h: 32,
					x: boss.x - (this.image_w * boss.c >> 1) + this.c * this.image_w + (this.image_w >> 1),
					y: boss.y - (this.image_h * boss.r >> 1) + this.r * this.image_h + (this.image_w >> 1),
					amode: A_MORTAL,
					move() {
						this.x = boss.x - (this.image_w * boss.c >> 1) + this.c * this.image_w + (this.image_w >> 1);
						this.y = boss.y - (this.image_h * boss.r >> 1) + this.r * this.image_h + (this.image_w >> 1);
	...
	vanish() {
		...
		if (animcounter % 8 == 0) setBakuen(boss.x - (this.c * 32 >> 1) + 16 + (Math.random() * this.c | 0) * 32, boss.y - (this.r * 32 >> 1) + 16 + (Math.random() * this.r | 0) * 32);
	}
};
ttamas = [];
